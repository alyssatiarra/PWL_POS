...
class TransaksiController extends Controller
{
    ...
    public function show(string $id)
    {
        ...
        $activeMenu = 'transaksi';
        return view('transaksi.show', ['breadcrumb' => $breadcrumb, 'page' => $page, 'penjualan' => $penjualan, 'activeMenu' => $activeMenu]);
    }
    ...
    {
        $transaksi = TransaksiModel::with(['user', 'detail.barang'])->find($id);
    ...
    {
        $transaksi = TransaksiModel::with(['detail.barang'])->find($id);
        $user = UserModel::select('user_id', 'nama')->get();
        $barang  = BarangModel::select('barang_id', 'barang_nama')->get();
    ...
    {
        $transaksi = TransaksiModel::with(['user', 'detail.barang'])->findOrFail($id);

        if (!$transaksi) {
            return redirect()->back()->with('error', 'Transaksi tidak ditemukan.');
        }

        $pdf = PDF::loadView('transaksi.export_detail_pdf', compact('transaksi'));
