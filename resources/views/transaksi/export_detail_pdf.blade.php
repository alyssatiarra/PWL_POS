...
<body>
    ...
    {{-- <h4>Tanggal Penjualan: {{ $transaksi->penjualan_tanggal->format('Y-m-d H:i:s') }}</h4> --}}
    <h4>Tanggal Penjualan: {{ \Carbon\Carbon::parse($transaksi->penjualan_tanggal)->format('Y-m-d') }}</h4>
    <table class="border-all">
        ...
        <tbody>
            @if ($transaksi->detail && count($transaksi->detail) > 0)
                @foreach ($transaksi->detail as $b)
                    <tr>
                        <td class="text-center">{{ $loop->iteration }}</td>
                        <td>{{ $b->barang->barang_nama ?? 'N/A' }}</td>
                        <td>{{ $b->jumlah }}</td>
